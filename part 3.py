...
def first_page():
    output = Registry().link_metadata()
    return render_template('metadata.html', result = output)

@app.route('/Sem') #general semantics
def second_page():
    output = Registry().link_semantics() # link from part 1
    return render_template('general_semantics.html', result = output) # html = template
...
def fourthuser_page():
     ...
     return render_template('user_genesentences.html', result = output) # input
